...
from cognitive_tribunal.utils.ui import (
    print_header, print_section, print_success, print_error, print_info, console
)
...
Examples:
  ...
  python main.py --ai-conversations /path/to/chatgpt/export --output-dir ./output
    ...
    print_header("COGNITIVE ARCHAEOLOGY TRIBUNAL", "Comprehensive Archaeological Dig Tool")
    ...
    if args.scan_archives:
        print_section("[1/4] Running Archive Scanner")
        
        with console.status("Scanning archives..."):
    ...
    if args.ai_conversations:
        print_section("[2/4] Running AI Context Aggregator")
        
        with console.status("Loading AI conversations..."):
    ...
    if args.personal_repos:
        print_section("[3/4] Running Personal Repo Analyzer")
        
        try:
            with console.status("Analyzing personal repos..."):
            ...
            print_success(f"Personal repo analysis complete. Analyzed {repo_results.get('stats', {}).get('total_repos', 0)} repositories")
        except Exception as e:
            print_error(f"Error analyzing personal repos: {e}")
    
    # Module 4: Org Repo Analyzer
    if args.org_repos:
        print_section("[4/4] Running Org Repo Analyzer")
        
        try:
            with console.status("Analyzing org repos..."):
            ...
            print_success(f"Org repo analysis complete. Analyzed {org_results.get('stats', {}).get('total_repos', 0)} repositories")
        except Exception as e:
            print_error(f"Error analyzing org repos: {e}")
    
    # Module 5: Web Bookmark Analyzer
    if args.web_bookmarks:
        print_section("[5/5] Running Web Bookmark Analyzer")

        with console.status("Analyzing bookmarks..."):
        ...
        print_success(f"Web bookmark analysis complete. Found {bookmark_results.get('stats', {}).get('total_bookmarks', 0)} bookmarks")
    ...
    if not args.no_inventory:
        print_info("Generating unified inventory...")
        with console.status("Building inventory..."):
    ...
    if not args.no_graph:
        print_info("Generating knowledge graph...")
    ...
    if not args.no_triage:
        print_info("Generating triage report...")
        with console.status("Analyzing results for triage..."):
    ...
    print_info(f"All outputs saved to: {output_dir.absolute()}")
    print("\nGenerated files:")
