...
# Conditional import for rich text interface
try:
...
def print_empty_state(parser):
    """Prints a friendly empty state using Rich if available."""
    ...
    # Title Panel
    title_text = Text("Cognitive Archaeology Tribunal", style="bold blue")
    subtitle_text = Text(
        "\nComprehensive Digital Auditing & Archiving Tool", style="italic cyan")

    welcome_panel = Panel(
        Text.assemble(title_text, subtitle_text, justify="center"),
        box=box.ROUNDED,
        padding=(1, 2),
        expand=False,
        border_style="blue"
    ...
    examples = [
        ("Run everything", "python main.py --all"),
        ("Scan archives", "python main.py --scan-archives ./data"),
        ("Analyze GitHub", "python main.py --personal-repos myuser"),
    ]
    ...
    console.print("\n[bold]Quick Start:[/bold]", justify="center")
    console.print(table, justify="center")
    console.print(
        "\n[dim]Run with --help for full documentation[/dim]", justify="center")
    console.print()
...
Examples:
  ...
  python main.py --ai-conversations /path/to/chatgpt/export --output-dir ./output
    ...
    if not (args.all or args.scan_archives or args.ai_conversations or args.personal_repos or args.org_repos or args.web_bookmarks):
        # UX: Show friendly empty state if no args provided instead of harsh error
        if len(sys.argv) == 1:
            print_empty_state(parser)
            sys.exit(0)
        else:
