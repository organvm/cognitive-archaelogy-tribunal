...
class ArchiveScanner:
    ...
    def is_unsafe_path(self, path: Path) -> bool:
        """
        Check if the path is a sensitive system directory or unsafe.
        ...
        """
        # Resolve to absolute path to catch symlinks
        try:
            abs_path = path.resolve()
        except (OSError, RuntimeError):
            # If we can't resolve it, it might be safer to reject or just check string
            abs_path = path.absolute()

        path_str = str(abs_path)

        # List of unsafe system directories (Unix/Linux based)
        # We check if the path is exactly one of these or a direct parent
        unsafe_roots = [
            '/',
            '/etc',
            '/var',
        ...
        for unsafe in unsafe_roots:
            if path_str == unsafe:
                return True
            # Also prevent scanning inside sensitive dirs like /etc/passwd if passed directly
            if path_str.startswith(unsafe + os.sep) and unsafe != '/':
                # Exception: /usr/local/src might be safe, but generally scanning /usr is bad.
                # For now, we are conservative.
                # Allow /var/tmp or /tmp if needed?
                # Let's keep it strict for now as per Sentinel's mission.
                return True
    ...
    def scan_directory(self, root_path: str, recursive: bool = True, max_depth: Optional[int] = None) -> Dict:
        ...
        if self.is_unsafe_path(root):
            return {'error': f"Security risk: Path is a sensitive system directory: {root_path}"}
