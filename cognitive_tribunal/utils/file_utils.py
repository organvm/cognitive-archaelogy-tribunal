...
class FileHasher:
    ...
    def compute_hash(file_path: Path, algorithm: str = 'sha256') -> str:
        ...
        if algorithm == 'xxhash' and XXHASH_AVAILABLE:
            ...
            # Fallback for when xxhash is requested but not available, or for other algos
            if algorithm == 'xxhash':
                algorithm = 'sha256'
        ...
        try:
            with open(file_path, 'rb') as f:
                ...
                # Using 64KB buffer for better I/O performance
    ...
    def add_file(self, file_path: Path, compute_full_hash: bool = False):
        ...
        try:
            ...
            if compute_full_hash or len(self.size_to_files[file_size]) > 1:
                # Use xxhash for deduplication if available, otherwise sha256 (default)
                algo = 'xxhash' if XXHASH_AVAILABLE else 'sha256'
                full_hash = FileHasher.compute_hash(file_path, algorithm=algo)
        ...
        for files in self.size_to_files.values():
            if len(files) > 1:
                ...
                for file_path in files:
                    # Use xxhash for deduplication if available, otherwise sha256 (default)
                    algo = 'xxhash' if XXHASH_AVAILABLE else 'sha256'
                    full_hash = FileHasher.compute_hash(
                        file_path, algorithm=algo)
