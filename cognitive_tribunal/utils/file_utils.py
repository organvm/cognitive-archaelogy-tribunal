    ...
    def compute_hash(file_path: Path, algorithm: str = 'sha256') -> str:
        ...
        try:
            with open(file_path, 'rb') as f:
                ...
                # OPTIMIZATION: Increased buffer size from 8KB to 64KB for better I/O performance
...
class Deduplicator:
    ...
    def __init__(self):
        ...
        # OPTIMIZATION: Cache file path to hash to avoid re-computing hashes
    ...
    def add_file(self, file_path: Path, compute_full_hash: bool = False):
        ...
        try:
            ...
            if compute_full_hash or len(self.size_to_files[file_size]) > 1:
                # Check cache first (though for new files it won't be there)
        ...
        for size, files in self.size_to_files.items():
            if len(files) > 1:
                ...
                for file_path in files:
                    # OPTIMIZATION: Use cached hash if available
                    if file_path in self.file_to_hash:
                        full_hash = self.file_to_hash[file_path]
                    else:
                        full_hash = FileHasher.compute_hash(file_path)
