    ...
    def compute_hash(file_path: Path, algorithm: str = 'sha256') -> str:
        ...
        try:
            with open(file_path, 'rb') as f:
                # Read in chunks to handle large files. 64KB is generally more efficient than 8KB.
...
class Deduplicator:
    ...
    def __init__(self):
        ...
        self.file_to_hash: Dict[Path, str] = {}  # Cache for computed hashes

    def _get_hash(self, file_path: Path) -> str:
        """Get or compute hash for a file, using cache if available."""
        if file_path in self.file_to_hash:
            return self.file_to_hash[file_path]
