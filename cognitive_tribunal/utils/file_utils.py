    ...
    def compute_hash(file_path: Path, algorithm: str = 'sha256') -> str:
        ...
        try:
            with open(file_path, 'rb') as f:
                ...
                # Using 64KB buffer for better performance
...
class Deduplicator:
    ...
    def _get_hash(self, file_path: Path) -> str:
        ...
        if path_str not in self._file_hash_cache:
            self._file_hash_cache[path_str] = FileHasher.compute_hash(file_path)
    ...
    def add_file(self, file_path: Path, compute_full_hash: bool = False):
        ...
        try:
            ...
            # Compute full hash only if explicitly requested.
            # We delay hashing for size collisions to find_duplicates() to avoid redundant work.
            if compute_full_hash:
                full_hash = self._get_hash(file_path)
    ...
    def find_duplicates(self) -> Dict[str, List[Path]]:
        ...
        for _, files in self.size_to_files.items():
            if len(files) > 1:
