...
class TestArchiveScanner(unittest.TestCase):
    """Test suite for the ArchiveScanner class."""
    ...
    def test_deduplicator_reset_between_locations(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir1:
            with tempfile.TemporaryDirectory() as temp_dir2:
                ...
                # Scan second location - should not include duplicates from
                # first location
                result2 = scanner.scan_directory(str(dir2))
                dedup_stats2 = result2['deduplication']['stats']

                # Each location should report only 1 file (no duplicates within
                # location)
                self.assertEqual(
                    dedup_stats1['total_files'],
                    1,
                    f"Expected 1 file in location 1, got {
                        dedup_stats1['total_files']}")
                self.assertEqual(
                    dedup_stats1['duplicate_files'],
                    0,
                    f"Expected 0 duplicates in location 1, got {
                        dedup_stats1['duplicate_files']}")

                self.assertEqual(
                    dedup_stats2['total_files'],
                    1,
                    f"Expected 1 file in location 2, got {
                        dedup_stats2['total_files']}")
                self.assertEqual(
                    dedup_stats2['duplicate_files'],
                    0,
                    f"Expected 0 duplicates in location 2, got {
                        dedup_stats2['duplicate_files']}")
        ...
        with tempfile.TemporaryDirectory() as temp_dir1:
            with tempfile.TemporaryDirectory() as temp_dir2:
                ...
                self.create_test_file(
                    dir1, "file1.txt", "Content for location 1")
                self.create_test_file(
                    dir1, "file2.txt", "More content for location 1")

                self.create_test_file(
                    dir2, "file3.txt", "Content for location 2")

                # Use scan_multiple_locations
                scanner = ArchiveScanner()
                results = scanner.scan_multiple_locations(
                    [str(dir1), str(dir2)])
                ...
                self.assertEqual(
                    loc1_results['stats']['total_files'],
                    2,
                    f"Expected 2 files in location 1, got {
                        loc1_results['stats']['total_files']}")

                # Location 2 should have 1 file (not 3 from accumulated state)
                self.assertEqual(
                    loc2_results['stats']['total_files'],
                    1,
                    f"Expected 1 file in location 2, got {
                        loc2_results['stats']['total_files']}")

                # Deduplication stats for location 2 should only count its own
                # file
                loc2_dedup = loc2_results['deduplication']['stats']
                self.assertEqual(
                    loc2_dedup['total_files'],
                    1,
                    f"Expected 1 file in location 2 dedup stats, got {
                        loc2_dedup['total_files']}")
